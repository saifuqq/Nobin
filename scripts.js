...
document.addEventListener('DOMContentLoaded', function() {
    // ตรวจสอบและลบสินค้าที่ไม่สมบูรณ์เมื่อโหลดหน้า
    if (typeof ProductValidator !== 'undefined') {
        ProductValidator.cleanupProducts();
    }

    // เพิ่มปุ่มบันทึกในแผงควบคุม
    if (typeof AdminStateManager !== 'undefined') {
        AdminStateManager.addSaveButton();
    }
    // --- BULK IMPORT/EXPORT ---
    ...
    window.showEditProductModal = function(mode, cat, idx) {
        ...
        if (mode === 'edit' && cat && typeof idx === 'number') {
            ...
            if (typeof ProductValidator !== 'undefined' && !ProductValidator.isValidProduct(prod)) {
                showToast('พบข้อมูลสินค้าไม่สมบูรณ์ กำลังลบรายการ...', 'warning');
...
function toggleAdminPanelModal() {
    ...
    if (adminPanelModal.classList.contains('hidden')) {
        ...
        if (AdminStateManager.hasUnsavedChanges) {
            showAlertDialog(
                'มีการเปลี่ยนแปลงที่ยังไม่ได้บันทึก',
                'คุณต้องการปิดแผงควบคุมโดยไม่บันทึกการเปลี่ยนแปลงหรือไม่?',
                () => {
                    adminPanelModal.classList.add('hidden');
                    adminPanelModal.classList.remove('flex');
                    renderProducts();
                    AdminStateManager.clearUnsavedChanges();
                },
                true
            );
        } else {
            ...
            item.innerHTML = `
                <div class="flex-grow">
                    <div class="flex items-center gap-2">
                        <b class="text-lg">${prod.baseName}</b>
                        <span class="px-2 py-1 rounded-full text-xs font-semibold ${isOutOfStock ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}">
                            ${isOutOfStock ? '❌ สินค้าหมด' : '✓ มีสินค้า'}
                        </span>
                    </div>
                    <div class="text-sm text-gray-500 mt-1">
                        ${prod.variants && prod.variants.length ? prod.variants.map(v => v.type + (v.price ? ' ' + v.price + '฿' : '')).join(', ') : ''}
                    </div>
                </div>
                <div class="flex gap-2 items-center mt-2 sm:mt-0">
                    <button 
                        onclick="stockManager.toggleProductStock('${cat}',${idx})" 
                        class="${isOutOfStock 
                            ? 'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700' 
                            : 'bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700'} 
                            text-white px-3 py-1.5 rounded-lg text-sm font-semibold transition-all duration-200 shadow-sm hover:shadow flex items-center gap-1">
                        ${isOutOfStock 
                            ? '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>แจ้งมีสินค้า' 
                            : '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>แจ้งสินค้าหมด'}
                    </button>
                    <button onclick="showEditProductModal('${cat}',${idx})" class="btn-info px-3 py-1.5 rounded-lg text-sm font-semibold flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                        แก้ไข
                    </button>
                    <button onclick="deleteProduct('${cat}',${idx})" class="btn-danger px-3 py-1.5 rounded-lg text-sm font-semibold flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
